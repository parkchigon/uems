...
function checkComplete(){
	...
	if(finishYn == 'N'){ //신규등록
		...
		<c:forEach var="subList" items="${SUB_DETAIL_INFO}" varStatus="subStatus">
			...
			var etcArr =[];
			var title = "${BASIC_INFO.facilityName}(${subList.pumpSubname})";
			var sortNo = "";
...
<div id="content_area">
	<c:if test="${SUB_DETAIL_INFO_SIZE > 0}">
		<c:forEach var="subList" items="${SUB_DETAIL_INFO}" varStatus="subStatus">
			<p class="subTitle_area">${subList.pumpSubname}
				...
				<div class="chart_area">
					<dl>
						<dt>${status.count}. ${question.question}
							<span class="fr">
								<a class="btn_camera" href="javascript:shootPicture('${question.msqId}','${subList.fpId}');"><span class="blind">사진촬영</span></a>
													...
													<c:choose>
														...
														<c:when test="${answer.checkTypeFlag eq 'N' }">
															<c:if test="${question.designedCode eq '01' }">
																<c:if test="${answer.answer eq 'mode'}">
																	<table class="data_row mt10">
																		...
																		<thead>
																			<tr>
																				<th scope="col" colspan="2"><strong>측정시 모드</strong></th>
																			</tr>
																		</thead>
																		<tbody>
																			<tr>
																				<th><input type="text" id="${answer.msaId}" name="result_${answer.msaId}_${subList.fpId}" value="${answer.result}" /></th>
																				
																			</tr>
																		</tbody>
																	</table>
																</c:if>
																<c:if test="${answer.answer eq 'check_power'}">
																	<table class="data_row mt20">
																		...
																		<thead>
																			<tr>
																				<th scope="col" colspan="2"><strong>소비전력[Kw] 측정</strong></th>
																			</tr>
																		</thead>
																		<tbody>
																			<tr>
																				<th>기준값</th>
																				<td><input type="text" value="${subList.spendPowerKw}" disabled="disabled"/></td>
																			</tr>
																			<tr>
																				<th>측정값</th>
																				<td><input type="text" id="${answer.msaId}" name="result_${answer.msaId}_${subList.fpId}" value="${answer.result}" /></td>
																			</tr>
																			<tr>
																				<th>확인기준</th>
																				<td><input type="text" value="MCC판넬확인" disabled="disabled" /></td>
																			</tr>
																		</tbody>
																	</table>
																</c:if>
																<c:if test="${answer.answer eq 'check_current'}">
																	<table class="data_row mt10">
																		<caption>caption</caption>
																		<colgroup>
																			<col width="30%">
																		...
																		<thead>
																			<tr>
																				<th scope="col" colspan="2"><strong>운전전류[A] 측정</strong></th>
																			</tr>
																		</thead>
																		<tbody>
																			<tr>
																				<th>기준값</th>
																				<td><input type="text" disabled="disabled" value="${subList.elecCurrent}" /></td>
																			</tr>
																			<tr>
																				<th>측정값</th>
																				<td><input type="text" id="${answer.msaId}" name="result_${answer.msaId}_${subList.fpId}" value="${answer.result}" /></td>
																			...
																			<tr>
																				<th>확인기준</th>
																				<td><input type="text" disabled="disabled" value="MCC판넬확인" /></td>
																			</tr>
																		</tbody>
																	</table>
																</c:if>
																<c:if test="${answer.answer eq 'check_flow'}">
																	<table class="data_row mt10">
																		<caption>caption</caption>
																		<colgroup>
																			<col width="30%">
																		...
																		<thead>
																			<tr>
																				<th scope="col" colspan="2"><strong>운전 유량[㎡/h] 확인</strong></th>
																			</tr>
																		</thead>
																		<tbody>
																			<tr>
																				<th>기준값</th>
																				<td><input type="text" disabled="disabled" value="${subList.flowRate1}" /></td>
																			</tr>
																			<tr>
																				<th>측정값</th>
																				<td><input type="text" id="${answer.msaId}" name="result_${answer.msaId}_${subList.fpId}" value="${answer.result}" /></td>
																			...
																			<tr>
																				<th>확인기준</th>
																				<td><input type="text" disabled="disabled" value="유량계 확인" /></td>
																			</tr>
																		</tbody>
																	</table>
																</c:if>
															...
															<c:if test="${question.designedCode eq '99' }">
																<li style="display:inline;">${answer.sortNo})
																	<input type="text" id="${answer.msaId}" name="result_${answer.msaId}_${subList.fpId}" value="${answer.result}" style="display:inline;" class="etc_${subList.fpId}_${answer.sortNo}"/>
															</c:if>
														</c:when>
									...
									<c:if test="${answer.msqId eq question.msqId}">
										<c:choose>
											...
											<c:when test="${answer.checkTypeFlag eq 'N' }">
												<c:if test="${question.designedCode eq '01' }">
													<c:if test="${answer.answer eq 'mode'}">
														<table class="data_row mt10">
															...
															<tbody>
																<tr>
																	<th><input type="text" id="${answer.msaId}" name="result_${answer.msaId}" value="${answer.result}" /></th>
														...
														<table class="data_row mt20">
															...
															<colgroup>
																...
																<tbody>
																	<tr>
																		<th>기준값</th>
																		<td><input type="text" disabled="disabled" value="${subList.spendPowerKw}" /></td>
																	</tr>
																	<tr>
																		<th>측정값</th>
																		<td><input type="text" id="${answer.msaId}" name="result_${answer.msaId}" value="${answer.result}" /></td>
																	</tr>
																	<tr>
																		<th>확인기준</th>
																		<td><input type="text" disabled="disabled" value="MCC판넬확인" /></td>
												...
												<c:if test="${question.designedCode eq '99' }">
													<li style="display:inline;">${answer.sortNo})
														<input type="text" id="${answer.msaId}" name="result_${answer.msaId}" value="${answer.result}" style="display:inline;"/>
												</c:if>
											</c:when>
											<c:otherwise></c:otherwise>
										</c:choose>
		...
		<c:forEach var="subList" items="${SUB_DETAIL_INFO}" varStatus="status">
			...
			<colgroup>
				...
				<thead>
					<tr>
						<th scope="col"></th>
						<th scope="col" colspan="2">${subList.pumpSubname}</th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<th rowspan="2">모델번호(펌프)</th>
						<td colspan="2">${subList.pumpModelName}</td>
					</tr>
					<tr>
						<td colspan="2">${subList.pumpModelNo} </td>
					</tr>
					<tr>
						<th rowspan="2">모델번호(모터)</th>
						<td colspan="2">${subList.motorModelName}</td>
					</tr>
					<tr>
						<td colspan="2">${subList.motorModelNo}</td>
					</tr>
					<tr>
						<th>전원</th>
					...
					<tr>
						...
						<td colspan="2">${subList.spendPowerKw}</td>
					</tr>
					<tr>
						<td colspan="2">${subList.spendPowerHp}</td>
					</tr>
					<tr>
						<th>전류</th>
						<td colspan="2">${subList.elecCurrent}</td>
					</tr>
					<tr>
						<th>유량(㎡/h/lpm)</th>
						<td>${subList.flowRate1}㎡/h</td>
						<td>${subList.flowRate2}pm</td>
					</tr>
					<tr>
						<th>양정</th>
						<td colspan="2">${subList.lift}</td>
					</tr>
					<tr>
						<th>유체</th>
						<td colspan="2">${subList.fluidMaterial} / ${subList.fluidPercent}</td>
					</tr>
					<tr>
						<th>모터형식</th>
						<td colspan="2">${subList.motorType}</td>
					</tr>
				</tbody>
			</table>
		</c:forEach>
