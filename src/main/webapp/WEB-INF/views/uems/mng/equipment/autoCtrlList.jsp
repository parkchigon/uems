...
<script type="text/javaScript">
		...
		if(isEmpty(sgId)) {
			...
			$.ajax({
				...
				for(var i=0; i<list.length; i++) {
					if(list[i].siteId == '${autoCtrl.searchCondition2}') {
	...
	function goRegist(famId){
		var f = document.frm;
		f.currPage.value = ${autoCtrl.currPage};
		f.famId.value = famId;
		f.action = "<c:url value='/uems/mng/equipment/autoCtrlRegist.do'/>";
		f.submit();
	}
	
	function goSearch(){
		var f = document.frm;
		f.action = "<c:url value='/uems/mng/equipment/autoCtrlList.do'/>"
		f.submit();
	}
</script>
...
<form:form commandName="autoCtrl" name="frm" id="frm" method="post">
	...
	<br><br>
	 ...
	 <div class="thead_wrap cboth">
		 <div class="ltr">
			<c:choose>
				<c:when test="${fn:length(autoCtrlList) eq 0}">
					<p class="tit_type01">총 : <strong class="imp">0</strong>개</p>
				</c:when>
				<c:otherwise>
					<p class="tit_type01">총 : <strong class="imp">${autoCtrlList[0].sTotalcount}</strong>개</p>
				</c:otherwise>	
			</c:choose> 
		</div>
	</div>
	...
	<div class="table_wrap">
		<table cellpadding="0" cellspacing="0" border="0" class="table_list_type">
			...
			<tbody>
				...
				<c:if test="${fn:length(autoCtrlList) eq 0}">
					<tr>
						<td colspan="6"> 등록된 게시물이 없습니다.</td>
					</tr>
				</c:if>
				<c:forEach var="result" items="${autoCtrlList}" >
					<tr>
						<td>${result.sPagelistno}</td>
						<td>${result.siteDesc}</td>
						<td><a href="javascript:goRegist('${result.famId}');" class="link_imp01">${result.autoctrlName}</a></td>
						<td>${result.form}</td>
						<td>${result.powerSupply}</td>
						<td>${result.salesInfo}</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
	...
	<div class="paging_wrap">
		<c:out value="${autoCtrl.pageutil.pagingBody}" escapeXml="false" />
	</div>
	
	<!-- e: 페이징 -->
</form:form>
 </div>
<c:out value="${autoCtrl.pageutil.pagingHtml}" escapeXml="false" />