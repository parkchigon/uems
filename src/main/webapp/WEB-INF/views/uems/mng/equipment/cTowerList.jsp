...
<script type="text/javaScript">
		...
		if(isEmpty(sgId)) {
			...
			$.ajax({
				...
				for(var i=0; i<list.length; i++) {
					if(list[i].siteId == '${cTower.searchCondition2}') {
	...
	/*펌프관리 등록화면 이동  */
	function goRegist(fcmId){
		var f = document.frm;
		f.fcmId.value = fcmId;
		f.currPage.value = ${cTower.currPage};
		f.action = "<c:url value='/uems/mng/equipment/cTowerRegist.do'/>";
		f.submit();
	}
	...
	function goSearch(){
		var f = document.frm;
		f.action = "<c:url value='/uems/mng/equipment/cTowerList.do'/>"
		f.submit();
	}
</script>
...
<form:form commandName="cTower" name="frm" id="frm" method="post">
	...
	<br><br>
	 ...
	 <div class="thead_wrap cboth">
		 <div class="ltr">
			<c:choose>
				<c:when test="${fn:length(cTowerList) eq 0}">
					<p class="tit_type01">총 : <strong class="imp">0</strong>개</p>
				</c:when>
				<c:otherwise>
					<p class="tit_type01">총 : <strong class="imp">${cTowerList[0].sTotalcount}</strong>개</p>
				</c:otherwise>	
			</c:choose> 
		</div>
	</div>
	...
	<div class="table_wrap">
		<table cellpadding="0" cellspacing="0" border="0" class="table_list_type">
			...
			<tbody>
				<c:if test="${fn:length(cTowerList) eq 0}">
					<tr>
						<td colspan="6"> 등록된 게시물이 없습니다.</td>
					</tr>
				</c:if>
				<c:forEach var="result" items="${cTowerList}" >
					<tr>
 						<td>${result.sPagelistno}</td>
						<td>${result.siteDesc}</td>
						<td><a href="javascript:goRegist('${result.fcmId}');" class="link_imp01">${result.ctowerName}</a></td>
						<td>${result.type}</td>
						<td>${result.count}</td>
	...
	<div class="paging_wrap">
		<c:out value="${cTower.pageutil.pagingBody}" escapeXml="false" />
	</div>
	
	<!-- e: 페이징 -->
</form:form>
 </div>
<c:out value="${cTower.pageutil.pagingHtml}" escapeXml="false" /> 