...
<script type="text/javaScript">
		...
		if(isEmpty(sgId)) {
			...
			$.ajax({
				...
				for(var i=0; i<list.length; i++) {
					if(list[i].siteId == '${heatExc.searchCondition2}') {
	...
	function goRegist(fhmId){
		var f = document.frm;
		f.currPage.value = ${heatExc.currPage};
		f.fhmId.value = fhmId;
		f.action = "<c:url value='/uems/mng/equipment/heatExcRegist.do'/>";
		f.submit();
	}
	
	function goSearch(){
		var f = document.frm;
		f.action = "<c:url value='/uems/mng/equipment/heatExcList.do'/>"
		f.submit();
	}
</script>
...
<form:form commandName="heatExc" name="frm" id="frm" method="post">
	...
	<br><br>
	 ...
	 <div class="thead_wrap cboth">
		 <div class="ltr">
			<c:choose>
				<c:when test="${fn:length(heatExcList) eq 0}">
					<p class="tit_type01">총 : <strong class="imp">0</strong>개</p>
				</c:when>
				<c:otherwise>
					<p class="tit_type01">총 : <strong class="imp">${heatExcList[0].sTotalcount}</strong>개</p>
				</c:otherwise>	
			</c:choose>
		</div>
	</div>
	...
	<div class="table_wrap">
		<table cellpadding="0" cellspacing="0" border="0" class="table_list_type">
			...
			<tbody>
				...
				<c:if test="${fn:length(heatExcList) eq 0}">
					<tr>
						<td colspan="5"> 등록된 게시물이 없습니다.</td>
					</tr>
				</c:if>
				<c:forEach var="result" items="${heatExcList}" >
					<tr>
						<td>${result.sPagelistno}</td>
						<td>${result.siteDesc}</td>
						<td><a href="javascript:goRegist('${result.fhmId}');" class="link_imp01">${result.heatexcName}</a></td>
	...
	<div class="paging_wrap">
		<c:out value="${heatExc.pageutil.pagingBody}" escapeXml="false" />
	</div>
	
	<!-- e: 페이징 -->
</form:form>
 </div>
 <c:out value="${heatExc.pageutil.pagingHtml}" escapeXml="false" />