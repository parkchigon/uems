...
<script type="text/javaScript">
		...
		if(isEmpty(sgId)) {
			...
			$.ajax({
				...
				for(var i=0; i<list.length; i++) {
					if(list[i].siteId == '${pump.searchCondition2}') {
	...
	/*펌프관리 등록화면 이동  */
	function goRegist(fpmId){
		var f = document.frm;
		f.currPage.value = ${pump.currPage};
		f.fpmId.value = fpmId;
		f.action = "<c:url value='/uems/mng/equipment/pumpRegist.do'/>";
		f.submit();
	}
	...
	function goSearch(){
		var f = document.frm;
		f.action = "<c:url value='/uems/mng/equipment/pumpList.do'/>"
		f.submit();
	}
</script>
...
<form:form commandName="pump" name="frm" id="frm" method="post">
	<input type ="hidden" id ="fpmId" name = "fpmId" value ="" />
	...
	<br><br>
	 ...
	 <div class="thead_wrap cboth">
		 <div class="ltr">
			<c:choose>
				<c:when test="${fn:length(pumpList) eq 0}">
					<p class="tit_type01">총 : <strong class="imp">0</strong>개</p>
				</c:when>
				<c:otherwise>
					<p class="tit_type01">총 : <strong class="imp">${pumpList[0].sTotalcount}</strong>개</p>
				</c:otherwise>	
			</c:choose> 
		</div>
	</div>
	...
	<div class="table_wrap">
		<table cellpadding="0" cellspacing="0" border="0" class="table_list_type">
			...
			<tbody>
				<c:if test="${fn:length(pumpList) eq 0}">
					<tr>
						<td colspan="6"> 등록된 게시물이 없습니다.</td>
					</tr>
				</c:if>
				<c:forEach var="result" items="${pumpList}" >
					<tr>
						<td>${result.sPagelistno}</td>
						<td>${result.siteDesc}</td>
						<td><a href="javascript:goRegist('${result.fpmId}');" class="link_imp01">${result.pumpName}</a></td>
						<td>${result.testRunDate}</td>
						<td>${result.form}</td>
						<td>
							<c:if test ="${result.amtPumpName1 ne null and result.amtPumpName1 ne ''}">
								${result.amtPumpName1} : ${result.amtPumpCnt1}EA
							</c:if>
							<c:if test ="${result.amtPumpName2 ne null and result.amtPumpName2 ne ''}">
								 ${result.amtPumpName2}: ${result.amtPumpCnt2}EA
							</c:if>
						</td>
					</tr>
				</c:forEach>
	...
	<div class="paging_wrap">
		<c:out value="${pump.pageutil.pagingBody}" escapeXml="false" />
	</div>
	
	<!-- e: 페이징 -->
</form:form>
 </div>
<c:out value="${pump.pageutil.pagingHtml}" escapeXml="false" /> 