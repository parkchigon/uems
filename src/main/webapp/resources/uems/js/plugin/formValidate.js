 ...
 * validate-nonSpcChar  : 특수문자 입력 방지(_-는 포함)
 *
 * 필요할 경우 추가할것~
 */
function validate() {
	...
	$('.validate-req, .validate-digits, .validate-tel-num, .validate-email, validate-ssn, .validate-companyno').each(function(i,o) {
		var val = $(o).val();
		...
		if(/validate-nonSpcChar/.test(classNames)) { // 특수문자 입력 방지(_-는 포함)
